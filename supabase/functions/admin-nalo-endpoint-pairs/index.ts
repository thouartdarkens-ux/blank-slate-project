...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "insert") {
      const required = ["name", "ussd_function_name", "ussd_function_url", "webhook_function_name", "webhook_function_url"];
      for (const r of required) {
        if (!payload[r]) {
          return new Response(JSON.stringify({ error: `${r} required` }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      return new Response(JSON.stringify({ pair: row }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
