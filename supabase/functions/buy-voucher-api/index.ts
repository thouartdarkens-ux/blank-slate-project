...
import { sendAlertSMS, sendVoucherSMS } from "../_shared/sms-client.ts";
...
const whatsappmessage = async(destination,wproduct,wserial,wpin)=>{
console.log("sending whatsapp message")  
const response = await fetch('https://api.chatbotsafrica.com/api/v1.0/send/message/template', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({
    apikey: "d863d448-ddf3-49cc-abd3-b959a6ded853",
    destination: destination,
    template_id: 1547507196757089,
    type: "text",
    placeholder: `${destination},${wproduct} checker,SERIAL:${wserial},PIN:${wpin},0538848199`,
    }),
  })
...
const fulfillTransaction = async (transaction)=>{
  ...
  try {
    const data = await whatsappmessage(phone_number,product,voucher.serial,voucher.pin
    );
    console.log(`Sent voucher ${voucher.serial}`);
  } catch (error) {
    console.error(`Error with voucher ${voucher.serial}:`, error);
  }
}
