...
// Nalo Solutions SMS sender (replaces mNotify sendAlertSMS/sendVoucherSMS)
const NALO_URL = 'https://sms.nalosolutions.com/smsbackend/Resl_Nalo/send-message/';
const NALO_USERNAME = 'Movaconsult';
const NALO_PASSWORD = '@@3@kuRxBU6PL85';
const NALO_SENDER_ID = 'MOVAconsult';
async function sendNaloSMS({ message, recipients,senderId }: { message: string; recipients: string[]; senderId?: string }) {
  console.log('📲 Nalo send ->', { recipients, sender: senderId, preview: message.slice(0, 80) });
  const results = [];
  for (const msisdn of recipients) {
    ...
    try {
      const res = await fetch(NALO_URL, {
        method: 'POST',
      ...
      if (!res.ok) {
        console.error('Nalo error', res.status, text);
        results.push({ msisdn, ok: false, status: res.status, response: text });
        continue;
      }
      let parsed;
      try { parsed = JSON.parse(text); } catch { parsed = { raw: text }; }
      results.push({ msisdn, ok: true, response: parsed });
    } catch (err) {
      console.error('Nalo request failed for', msisdn, err);
      results.push({ msisdn, ok: false, error: (err as Error).message });
    }
  }
  return { results };
}
const sendAlertSMS = sendNaloSMS;
const sendVoucherSMS = sendNaloSMS;
